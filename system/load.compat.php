...
if(!isset($_REQUEST['view']) || empty($_REQUEST['view']) || is_array($_REQUEST['view']))
{
	$_REQUEST['view'] = "news";
}
else
	$_REQUEST['view'] = (string) $_REQUEST['view'];

if(Functions::isValidFolderName($_REQUEST['view']))
{
	if(Website::fileExists("pages/" . $_REQUEST['view'] . ".php"))
	{
		$subtopic = $_REQUEST['view'];
	}
	else
		$subtopic = '404';
		//new Error_Critic('CRITICAL ERROR', 'Cannot load page <b>' . htmlspecialchars($_REQUEST['view']) . '</b>, file does not exist.');
}
else
	$subtopic .= '404';
	//new Error_Critic('CRITICAL ERROR', 'Cannot load page <b>' . htmlspecialchars($_REQUEST['view']) . '</b>, invalid file name [contains illegal characters].');
...
//###################### FUNCTIONS ######################
function isBetaKeyValid($SQL, $key)
{
	$key = (string) $key;
	$betakey = $SQL->query("SELECT `id` FROM `beta_keys` WHERE `key` = " .$SQL->quote($key))->fetchAll();
	if (!$betakey) {
		return false;
	}
...
function hasBetaKeyBeenUsed($SQL, $key)
{
	$key = (string) $key;
	$betakey = $SQL->query("SELECT `id` FROM `beta_keys` WHERE `used` = '1' AND `key` = " .$SQL->quote($key))->fetchAll();
	if ($betakey) {
		return true;
	}
...
function getOutfitValue($lookType, $addon)
{
	return ($lookType << 16) | $addon;
}
...
function getValidPlayerName($name)
{
	$tmpName = '';
	if (!empty($name)) {
		$tmpPlayer = getPlayer($name);
		$tmpName = $tmpPlayer ? '<a href="?view=characters&name='.urlencode($tmpPlayer->getName()).'">'.htmlspecialchars($tmpPlayer->getName()).'</a>' : $name;
	}
...
function getGuild($id)
{
	$guild = new Guild();
	$guild->load($id);
	return $guild->isLoaded() ? $guild : null;
}

function getValidGuildName($id, $name)
{
	$tmpName = '';
	if (!empty($name)) {
		$tmpGuild = getGuild($id);
		$tmpName = $tmpGuild ? '<a href="?view=guilds&action=show&guild='.urlencode($tmpGuild->getID()).'">'.htmlspecialchars($tmpGuild->getName()).'</a>' : $name;
	}
