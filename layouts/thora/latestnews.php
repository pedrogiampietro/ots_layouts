...
function replaceAll($text, $smile)
{
	...
	while(stripos($text, '[size]') !== false && stripos($text, '[/size]') !== false )
    {
        $url = substr($text, stripos($text, '[size]')+5, stripos($text, '[/size]') - stripos($text, '[size]') - 5);
        $text = str_ireplace('[size]'.$url.'[/size]', '<font size="'.$url.'">'.$url.'</font>', $text);
    }
...
function showPost($topic, $text, $smile)
{
    ...
    {
		// only this code has been edited -- Hemrenus321
		// START{
        foreach($last_threads as $thread)
        {
            $main_content .= '<div class="news_title">' . htmlspecialchars($thread['post_topic']) . '<div class="date">'.date('d.m.y', $thread['post_date']).'</div></div>';
            $main_content .= '<div class="news_mid">' . showPost('', $thread['post_text'], $thread['post_smile']);
			$main_content .= '</div>';
			$main_content .= '<div class="news_bot"></div>';
	   }
		//  }END
    }
    else
