...
$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="top">Ranking for '.htmlspecialchars($list_name).'</h3></div><div class="body"><div class="text-center"><div class="btn-group">';
	foreach ($tmpSkills as $k => $v) {
		$main_content .= '<a href="?view=highscores&skill='.$k.'" class="btn btn-default '.($list == $k ? 'active' : '') .'">'.$v.'</a>';
	}

$main_content .= '</div></div><table class="table table-striped table-condensed table-bordered"><thead><tr><th width="5%">#</th><th>Name</th><th width="25%">Vocation</th><th width="15%">'. ($list == "level" ? 'Level' : ($list == "magic" ? 'Magic Level' : 'Skill')) .'</th></tr></thead><tbody>';
