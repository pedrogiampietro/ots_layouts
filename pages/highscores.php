...
$grupacc = "1,2,3,6"; //Seleciona os grupos de class que ir?o aparecer no rank
...
$main_content = '
   <form action="./?view=highscores" method="POST">
        ...
        if ($list == 5) {
            $main_content .= '
                                            <tr>
                                                ...
                                                <td><a href="./?view=characters&name=' . urlencode($skill["name"]) . '">' . htmlspecialchars($skill["name"]) . '</a></td>
                                                <td>' . $voc . '</td>
...
for ($i = 0; $i < $tp; $i++) {
    if((int)$_REQUEST["page"]-1 != $i){
        $main_content .= '<a style="margin-left:4px;" href="./?view=highscores&world=' . $config["server"]["serverName"] . '&vocation=' . $vocation . '&page=' . ($i + 1) . '">' . ($i + 1) . '</a>';
    }else{
