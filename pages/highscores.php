...
$main_content .= '<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title">Ranking for '.htmlspecialchars($list_name).'</h3></div><div class="panel-body"><div class="text-center">';
	foreach ($tmpSkills as $k => $v) {
		$main_content .= '<a href="?view=highscores&skill='.$k.'" class="btn btn-primary '.($list == $k ? 'active' : '') .'">'.$v.'</a>';
	}

$main_content .= '</div></div><table class="table table-striped"><thead><tr><th width="5%">#</th><th>Name</th><th width="25%">Vocation</th><th width="15%">'. ($list == "level" ? 'Level' : ($list == "magic" ? 'Magic Level' : 'Skill')) .'</th></tr></thead><tbody>';
