...
if(count($config['site']['worlds']) > 1)
{
	...
	<FORM ACTION="" METHOD=get><INPUT TYPE="hidden" NAME="subtopic" VALUE="houses"><TABLE WIDTH=100% BORDER=0 CELLSPACING=1 CELLPADDING=4><TR><TD BGCOLOR="'.$config['site']['painel'].'" CLASS=white><B>World Selection</B></TD></TR><TR><TD BGCOLOR="'.$config['site']['primeira_td'].'">
...
$main_content .= '<form action="" method="GET"><input type="hidden" name="subtopic" value="houses" /><table width="100%"><tr style="background-color:' . $config['site']['painel'] . ';font-weight:bold"><td style="color:white">Town</td><td style="color:white">Owner</td><td style="color:white">Sort by</td></tr>';
...
$main_content .= '</td></tr>';
$main_content .= '<tr><td colspan="3" style="text-align:right"><input type="image" name="Submit" alt="Submit" src="'.$layout_name.'/assets/images/sbutton_submit.gif"></td></tr></table></form>';
...
if(isset($_REQUEST['town']) && isset($_REQUEST['owner']) && isset($_REQUEST['order']))
{
	...
	$main_content .= '<table width="100%"><tr style="background-color:' . $config['site']['painel'] . ';font-weight:bold"><td style="color:white">Name</td><td style="color:white">Owner</td><td style="color:white">Size</td></tr>' . $housesText . '</table>';
}