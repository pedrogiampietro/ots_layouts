...
$main_content .= '
	<div class="panel panel-default">
		...
		<div class="panel-body">';
			...
			$adminList = $SQL->query('SELECT ' . $SQL->fieldName('name') . ', ' . $SQL->fieldName('lastlogin') . ', ' . $SQL->fieldName('id') . ', ' . $SQL->fieldName('group_id') . ' FROM ' . $SQL->tableName('players') . ' WHERE ' . $SQL->fieldName('group_id') . ' >= 5');
			$main_content .= '<table class="table table-striped table-condensed"><thead><tr><th style="width: 80%">Administrators</th><th>Status</th></tr></thead><tbody>';
			...
			$main_content .= '<table style="margin-top: 15px;" class="table table-striped table-condensed"><thead><tr><th style="width: 80%;">Tutors</th><th>Status</th></tr></thead><tbody>';
				foreach($adminList as $i => $admin) {
					$main_content .= '<tr><td><a href="?view=characters&name='.urlencode($admin['name']).'">'.htmlspecialchars($admin['name']).'</a></td><td><span class="label label-info">Last Seen: ' . (($admin['lastlogin'] > 0) ? date("j M Y, H:i:s", $admin['lastlogin']) : 'Never') . '</span></td></tr>';
				}
...
$main_content .= '
			...
			<br>
			<p>If no one is available to help you, you can also send an e-mail in Portuguese or English to <span class="label label-info">thoriaonline@gmail.com</span> and explain what you need help with. We will try to respond within 48 hours.</p>
		</div>
	</div>
';
