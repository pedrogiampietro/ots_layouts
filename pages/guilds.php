...
if ($action == '')
{
	...
	$main_content .= '<header>Guilds</h3></div><div class="panel-body">';

	$guilds_list = new DatabaseList('Guild');
	$guilds_list->addOrder(new SQL_Order(new SQL_Field('name'), SQL_Order::ASC));

	if (count($guilds_list) > 0) {
		$main_content .= '<table id="searchArmoryResults"><thead><tr><th>Name</th><th width="20%">Members</th><th width="20%">Average Level</th></tr></thead><tbody>';
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<?php if ($players_from_account_in_guild > 0 && ($guild_vice || $guild_leader)) { ?>
					<div class="btn-group pull-right" style="top: 15px;position:relative;">

					</div>
				<?php } ?>
				<header>Guild: <?php echo $guild->getName(); ?></h3>
			</div>
...
if ($action == 'createguild')
{
	...
	if ($todo == 'save')
	{
		...
		$main_content .= '<div class="panel panel-default"><div class="panel-heading"><header>Create Guild</h3></div><div class="panel-body"><p>Congratulations!</p><p>You have created guild <b>'.htmlspecialchars($new_guild_name).'</b>. <b>'.htmlspecialchars($player->getName()).'</b> is leader of this guild. Now you can invite players, change picture, description and motd of guild. Press submit to open guild manager.</p><div class="text-center"><a class="btn btn-sm btn-primary" href="?view=guilds&action=show&guild='.$new_guild->getId().'">View</a></div></div></div>';
	}
	else
	{

		$main_content .= '<div class="panel panel-default"><div class="panel-heading"><header>Create Guild</h3></div><div class="panel-body">
...
if ($action == 'changedescription')
{
	...
	if (empty($guild_errors) && $_REQUEST['todo'] == 'save') {
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Guild Description</h3></div>
				<div class="panel-body">
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Edit Guild Description for <b>'.htmlspecialchars($guild->getName()).'</b></h3></div>
				<div class="panel-body">
...
if ($action == 'passleadership')
{
	...
	if (empty($guild_errors) && empty($guild_errors2) && $_POST['todo'] == 'save') {
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Pass Leadership</h3></div>
				<div class="panel-body">
...
if ($action == 'deleteguild')
{
	...
	if (empty($guild_errors) && $_REQUEST['todo'] == 'save') {
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Guild Disbanded</h3></div>
				<div class="panel-body">
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Disband Guild</h3></div>
				<div class="panel-body">
...
if ($action == 'changenick') {
	...
	if (empty($errors) && $_REQUEST['todo'] == 'save') {
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Nickname for '.htmlspecialchars($player->getName()).' changed</h3></div>
				<div class="panel-body">
		...
		$main_content .= '
			<div class="panel panel-default">
				<div class="panel-heading"><header>Edit nickname for <b>'.htmlspecialchars($player->getName()).'</b></h3></div>
				<div class="panel-body">
	...
	if (empty($guild_errors)) {
		...
		if (empty($guild_errors)) {
			...
			if ($guild_leader) {
				...
				$main_content .= '
					<div class="panel panel-default">
						<div class="panel-heading"><header>War Declaration</h3></div>
						<div class="panel-body">
