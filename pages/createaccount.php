...
function EmailStateChanged() 
{
	...
	$main_content .= 'To play on '.htmlspecialchars($config['server']['serverName']).' you need an account. 
						...
						<fieldset><legend><B>Create an '.htmlspecialchars($config['server']['serverName']).' Account</B></legend></fieldset>
						<TR><TD BGCOLOR="'.$config['site']['secondcolor'].'"><TABLE BORDER=0 CELLSPACING=8 CELLPADDING=0>
						  <TR><TD>
	...
	$main_content .= '</TEXTAREA>
					...
					<TABLE BORDER=0 WIDTH=100%>
					  <TR><TD ALIGN=center>
					    ...
					    <INPUT TYPE=submit value="I Agree" BORDER=0 WIDTH=120 HEIGHT=18>
					    </FORM>
					  </TD><TD ALIGN=center>
					    <FORM  ACTION="?subtopic=latestnews" METHOD=post>
					    <INPUT TYPE=submit value="Cancel" BORDER=0 WIDTH=120 HEIGHT=18>
					    </FORM>
					  </TD><TD ALIGN=center>
	...
	if(empty($reg_form_errors))
	{
		...
		if($config['site']['send_emails'] && $config['site']['create_account_verify_mail'])
		{
			...
			if($mail->Send())
			{
				...
				<TR><TD BGCOLOR="'.$config['site']['firstcolor'].'" CLASS=white><B>Account Created</B></TD></TR>
