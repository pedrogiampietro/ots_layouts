...
class Player extends ObjectData
{
	...
	public function isInGuild($guildId)
	{
		$rank = $this->getRank();
		if ($rank) {
			if ($rank->getGuild()->getId() == $guildId) {
				return true;
			}
		}
...
*/
	...
	public function getOnlineTime(){return $this->data['onlinetime'];}
...
*/
	...
	public function hasOutfit($lookType, $addon)
	{
		$value = getOutfitValue($lookType, $addon);

		$SQL = Website::getDBHandle();
		$query = $SQL->query('SELECT * FROM ' . $SQL->tableName('player_storage') . ' WHERE ' . $SQL->fieldName('player_id') . ' = ' . $SQL->quote(self::getId()) . ' AND ' . $SQL->fieldName('key') . ' > 10001000 AND ' . $SQL->fieldName('key') . ' < 10002000 AND ' . $SQL->fieldName('value') . ' = ' . $SQL->quote($value))->fetch();
		return $query !== false;
	}

	public function addOutfit($lookType, $addon)
	{
		$SQL = Website::getDBHandle();
		$count = $SQL->query('SELECT COUNT(' . $SQL->fieldName('key') . ') AS `count` FROM ' . $SQL->tableName('player_storage') . ' WHERE ' . $SQL->fieldName('player_id') . ' = ' . $SQL->quote(self::getId()) . ' AND ' . $SQL->fieldName('key') . ' > 10001000 AND ' . $SQL->fieldName('key') . ' < 10002000;')->fetch();
		$key = 10001001 + (int) $count['count'];
		self::setStorageValue($key, getOutfitValue($lookType, $addon));
	}

	public function hasMount($mountId)
	{
		$tmpMountId = $mountId - 1;
		$key = floor(10002001 + ($tmpMountId / 31));

		$storageValue = self::getStorageValue($key);
		$value = $storageValue != -1 ? $storageValue : 0;
		return ((1 << ($tmpMountId % 31)) & $value) != 0;
	}

	public function addMount($mountId)
	{
		$tmpMountId = $mountId - 1;
		$key = floor(10002001 + ($tmpMountId / 31));
		$value = self::getStorageValue($key);
		if ($value != -1) {
		...
		$firstMount = self::getStorageValue(10002011);
		if ($firstMount != -1) {
			self::setStorageValue($key, $value);
		} else {
			self::setStorageValue($key, $value);
			self::setStorageValue(10002011, $mountId);
		}
	}

	public function hasDeaths()
	{
		$SQL = Website::getDBHandle();
		$query = $SQL->query('SELECT * FROM ' . $SQL->tableName('player_deaths') . ' WHERE ' . $SQL->fieldName('player_id') . ' = ' . $SQL->quote(self::getId()))->fetch();
		return $query !== false;
	}
	...
	{
		$SQL = Website::getDBHandle();
		$query = $SQL->query('SELECT ' . $SQL->fieldName('value') . ' FROM ' . $SQL->tableName('player_storage') . ' WHERE ' . $SQL->fieldName('key') . ' = ' . $SQL->quote($key) . ' AND ' . $SQL->fieldName('player_id') . ' = ' . $SQL->quote(self::getId()))->fetch();
		if ($query !== false)
			return $query['value'];
	...
	{
		$SQL = Website::getDBHandle();
		if (self::getStorageValue($key) != -1) {
			$SQL->query('UPDATE ' . $SQL->tableName('player_storage') . ' SET ' . $SQL->fieldName('value') . ' = ' . $SQL->quote($value) . ' WHERE ' . $SQL->fieldName('key') . ' = ' . $SQL->quote($key) . ' AND ' . $SQL->fieldName('player_id') . ' = ' . $SQL->quote(self::getId()));
		} else {
			$SQL->query('INSERT INTO ' . $SQL->tableName('player_storage') . ' (' . $SQL->fieldName('player_id') . ', ' . $SQL->fieldName('key') . ', ' . $SQL->fieldName('value') . ') VALUES (' . $SQL->quote(self::getId()) . ', ' . $SQL->quote($key) . ', ' . $SQL->quote($value) . ')');
		}
	}

	public function getAchievements($achievements)
	{
		$tmpAchievements = array();
		foreach ($achievements as $key => $achievement) {
			if (self::getStorageValue($key) != -1) {
				array_push($tmpAchievements, array(
						'name' => $achievement['name'],
						'grade' => $achievement['grade'],
						'points' => $achievement['points'],
						'description' => $achievement['description'],
						'secret' => (isset($achievement['secret']) ? $achievement['secret'] : false)
					)
				);
			}
		}

		return $tmpAchievements;
	}

	public function getAchievementPoints($achievements)
	{
		$points = 0;
		$list = self::getAchievements($achievements);
		if (count($list) != 0) {
			foreach ($list as $achievement) {
				if (isset($achievement['points']) && $achievement['points'] > 0) {
					$points += $achievement['points'];
				}
			}
		}
	...
	{
		$tmpOutfits = array();
		for ($addon = 0; $addon < 4; $addon++) {
			foreach ($outfits as $name => $outfit) {
				if (self::hasOutfit($outfit['male'], $addon) && self::hasOutfit($outfit['female'], $addon)) {
					array_push($tmpOutfits, array(
						'name' => $name,
						'male' => $outfit['male'],
	...
	public function getMounts($mounts)
	{
		$tmpMounts = array();
		foreach ($mounts as $mountId => $mount) {
			if (self::hasMount($mountId)) {
				array_push($tmpMounts, array(
					'name' => $mount['name'],
	...
	{
		return self::getSex() == 0 ? 'female' : 'male';
	}
}
